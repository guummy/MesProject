...
<script>
function input1(rec_schedule_cd,wh_cd,product_cd_name, rec_date, pchor_cd,stock_count){
	
	opener.setChildValue(rec_schedule_cd,wh_cd,product_cd_name,rec_date,pchor_cd,stock_count);
...
<!-- 					<th>입고수량</th> -->
			...
			<tbody>
				...
				<tr style="text-align: center; font-size: 0.9rem">
					...
					<td>${receiveDTO.wh_cd}<br>${receiveDTO.wh_name}</td>
					<td>${receiveDTO.product_cd_name}<br>${receiveDTO.product_name}</td>
					<td>${receiveDTO.rec_date}</td>
					<td>${receiveDTO.stock_count}</td>
<%-- 					<td>${receiveDTO.rec_count}</td> --%>
					<td>${receiveDTO.pchor_cd}</td>
					<td><button class="button2" onClick="input1('${receiveDTO.rec_schedule_cd}','${receiveDTO.wh_cd}','${receiveDTO.product_cd_name}','${receiveDTO.rec_date}','${receiveDTO.pchor_cd}','${receiveDTO.stock_count}');">선택</button></td>
				</tr>
			</c:forEach>	
			</tbody>
		</table>
